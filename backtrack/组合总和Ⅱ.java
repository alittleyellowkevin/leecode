...
//    }
//
//    private static void backtrack(HashMap<Integer, Boolean> map, int[] candidates, ArrayList<List<Integer>> res,int j, int result, ArrayList<Integer> number) {
//        if(result == 0){
//            ArrayList<Integer> numberclone = new ArrayList<>();
//            numberclone = (ArrayList<Integer>) number.clone();
//            if(j != candidates.length-1 && map.get(j+1) == false && candidates[j] == candidates[j+1]){
//                return;
//            }
//            res.add(numberclone);
//
